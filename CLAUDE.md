...
7. `result_writer.write_results_to_iceberg` — `CREATE TABLE IF NOT EXISTS` on `{dl_layer}_dqf_{domain}_results` (Iceberg, partitioned by `execution_date`, merge-on-read) then `df.writeTo(fqn).append()`. Skipped when `results_write_enabled=False`. When `results_table_location` is set, the `dl_layer` prefix must appear in the directory name too, otherwise two scopes would share one warehouse directory.
