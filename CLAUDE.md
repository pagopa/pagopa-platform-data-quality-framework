...
Note: the CI lint gate is `ruff check .` (`.github/workflows/dq_framework_ci.yml`); CI does **not** run pytest. The Makefile `make lint` target still calls `flake8`, which is *not* what CI enforces — lint with `ruff check .` to match CI.

`ruff` is **pinned** (`ruff==0.16.5`) in the workflow and its rule set is declared explicitly in `[tool.ruff.lint]`: `select = ["E", "W", "F", "I", "UP"]`, `ignore = ["UP045"]`, with `line-length = 120` under `[tool.ruff]`. Both the pin and the explicit `select` are deliberate: ruff's *default* rule set widens across releases, and an unpinned `pip install ruff` turned the gate red with no code change at all. Do not drop either. `UP045` is off as a style decision — `Optional[X]` is preferred over `X | None` in annotations — so expect both forms in the tree; `DTZ`, `BLE`, `G`, `PERF` and `SIM` are outside the perimeter on purpose (see the comment in `pyproject.toml` for the reason on each).

### CI/CD runners

Both workflows run on a **self-hosted CodeBuild runner**, not on `ubuntu-latest`: `runs-on` is the label `codebuild-pagopa-platform-data-quality-framework-runner-${{ github.run_id }}-${{ github.run_attempt }}` (project in AWS account `407424444552`, region `eu-south-1`; `run_id`/`run_attempt` are required so CodeBuild binds the ephemeral runner to the one job). Consequences when editing a workflow:

- **Never use `actions/setup-python`.** Its prebuilt interpreters are built for Ubuntu and fail on the runner's `amazonlinux-x86_64-standard` image. Use the image's `python3` and build an explicit `.venv` (`requires-python` is `>=3.10`, so there is nothing to pin). Ruff's default excludes cover `.venv`, so `ruff check .` ignores it.
- **No `sudo`** — the container already runs as root and `sudo` is not guaranteed in the image.
- **No Docker**: the project has `privilegedMode: false`, so container-based actions do not run. This is why `check_pr.yml` deliberately stays on `ubuntu-latest` — it uses container actions and its jobs only call the GitHub API, so a CodeBuild container would add cost for no gain.
- The runner project must stay **detached from any VPC**. It was once attached to the default VPC, which routes `0.0.0.0/0` to an Internet Gateway with no NAT; since CodeBuild VPC builds never get a public IP, every build died downloading the runner with `curl` exit 28.
- `dq_framework_cd.yml` is **`workflow_dispatch`-only**, gated by a `confirm` choice input (`yes`/`no`, quoted in YAML so they stay strings). A push no longer deploys to CDE.
...
2. `utils.incremental.apply_incremental_conditions` — if the SodaCL contains `${INCREMENTAL_CONDITIONS}`, resolves the watermark **per check** and substitutes it; otherwise a no-op. Priority: (a) `--watermark-from`, which wins over everything and applies the same value to every incremental check with no DB query at all; (b) a per-check lookup of `MAX(watermark_to)` on Iceberg, minus `incremental_lookback_minutes`; (c) `--watermark-bootstrap-from`, used **only** for the checks the lookup found nothing for. There is **no implicit epoch bootstrap**: if step (c) is reached and the flag is absent, it raises `ValueError` naming the specific check. The lookup reads the same `{dl_layer}_dqf_{domain}_results` table the writer targets, so two layers have independent watermarks.

   Two details that are easy to get wrong. First, the lookup does *not* filter on `outcome = 'pass'`: it filters on the outcomes of `AppConfig.incremental_watermark_advance_policy`, whose default is `executed` → `('pass', 'warn', 'fail')`, so a check that ends in warn or fail **still advances** its watermark and that window is never re-examined. This is a customer requirement, not an oversight — `pass_only` → `('pass',)` is the other supported policy, and an unrecognized value raises `ValueError` rather than degrading to a default. Second, a **missing results table is not an error**: `TABLE_OR_VIEW_NOT_FOUND` is logged as a warning and returns `None`, so the run falls through to `--watermark-bootstrap-from` (this is what happens on a first run for a new domain/layer, or right after a rename). `RuntimeError` is reserved for lookups that fail for *any other* reason — permissions, incompatible schema, unreachable catalog — where there is deliberately no fallback, so a real failure cannot turn into a silent full reprocess.
