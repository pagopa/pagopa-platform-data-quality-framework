...
```bash
ENV=dev python -m dq_framework.entrypoints.run_quality --domain gpd --watermark-from 1970-01-01
```

`--domain` è obbligatorio (determina il nome delle tabelle di output); `--watermark-from 1970-01-01` fa partire i controlli incrementali dall'epoch. In `dev` la scrittura su DB è disattivata (`results_write_enabled=False`): gli esiti vengono solo loggati, con il riepilogo dei check a fine run. Per provare invece il download del contratto da GitHub c'è `make run-dev-github`.
